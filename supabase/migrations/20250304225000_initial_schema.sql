-- Trip to Sardinia: initial schema and reference data
-- Idempotent: safe to run against an existing DB (IF NOT EXISTS / ON CONFLICT DO NOTHING).
-- Admin users are NOT seeded here; create them with: npm run create-admin (use a strong password in production).
...
-- Admin users (for dashboard login; passwords hashed with bcrypt via create-admin script)
...
-- Reference data: areas and categories (idempotent)
